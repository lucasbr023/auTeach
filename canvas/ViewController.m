...
@interface ViewController ()< UIGestureRecognizerDelegate, PBJVisionDelegate, UIAlertViewDelegate, UIImagePickerControllerDelegate >
...

{
    ...
    ALAssetsLibrary *CoisaDeSalvarEmAlbunsCoisados;
}
...
//Ph
-(void) sendToAnotherView{
    if(printPronto && videoPronto){
        _proximaView.hidden=NO;
        /*results *passData = [[results alloc] init];
        passData.imagem = printPronto;
        passData.videoURL = videoPronto;
        //[passData sendDataToAnotherView:printPronto videoURL:videoPronto];
        //[passData recordData:printPronto video:videoPronto];
        NSLog(@"testando, w -> %f, %f",printPronto.size.width,passData.imagem.size.width);
        [self.navigationController pushViewController:passData animated:YES];*/
        //NSLog(@"view controller, %f",printPronto.size.width);
        //[self.navigationController pushViewController:[self.storyboard instantiateViewControllerWithIdentifier:@"resultados"] animated:YES];
        //[self performSegueWithIdentifier:@"resultados" sender:self];
        //ViewController *_parentViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"resultados"];
        //[self presentViewController:_parentViewController animated:YES completion:nil];
        //[self presentModalViewController: animated:YES];
        //[self present]
    }
    
}
-(void) saveImageToCustomAlbum:(UIImage *)imageToSave{
    //---
    [CoisaDeSalvarEmAlbunsCoisados saveImage:imageToSave toAlbum:@"Jon" completion:nil failure:nil];
    printPronto = imageToSave;
    [self sendToAnotherView];
    //---
}
-(void) saveVideoToCustomAlbum: (NSURL *)video{
    [CoisaDeSalvarEmAlbunsCoisados saveVideo:video toAlbum:@"Jon" completion:nil failure:nil];
    videoPronto = video;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    NSLog(@"Preparando");
    results *passData = [segue destinationViewController];
    passData.imagem = printPronto;
    passData.videoURL = videoPronto;
    NSLog(@"testando, w -> %f, %f",printPronto.size.width,passData.imagem.size.width);
}
...
- (void)viewDidLoad
{
    _proximaView.hidden=YES;
    CoisaDeSalvarEmAlbunsCoisados = [[ALAssetsLibrary alloc] init];
    red = 0.0/255.0;
    green = 0.0/255.0;
    blue = 0.0/255.0;
    brush = 10.0;
...
- (void)vision:(PBJVision *)vision capturedVideo:(NSDictionary *)videoDict error:(NSError *)error
{
    ...
    //Ph!
    //videoPronto = [NSURL URLWithString:videoPath];
    NSLog(@"video url -> %@",videoPath);
    [self saveVideoToCustomAlbum:[NSURL URLWithString:videoPath]];
...
-(void)writing{
    ...
    while(!append_ok && j < 10){
        if(adaptor.assetWriterInput.readyForMoreMediaData){
            ...
            NSLog(@"Adptor not ready, %lu, %d",frameCount,j);
            [NSThread sleepForTimeInterval:0.0055];
        }
        j++;
    }
...
- (IBAction)save:(id)sender {
    ...
    UIGraphicsEndImageContext();
    //printPronto = SaveImage;
    
    [self saveImageToCustomAlbum:SaveImage];
    ...
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@""
                                               ...
                                               destructiveButtonTitle:nil
                                                    otherButtonTitles:@"Save in your album", @"Cancel", nil];
